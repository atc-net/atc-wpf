...
public static class GenerateHelper
{
    ...
    public static IList<LogKeyValueItem> GenerateFontAwesome7Brand(
        DirectoryInfo resourcesFolder,
        DirectoryInfo outputEnumFolder)
    {
        ArgumentNullException.ThrowIfNull(resourcesFolder);
        ...
        return GoV7CssParseAndGenerate(
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"css\fontawesome7-brands.css")),
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"fonts\fa7-brands-400.otf")),
            new FileInfo(Path.Combine(outputEnumFolder.FullName, @"FontAwesomeBrand7Type.cs")),
            "fa");
    }

    public static IList<LogKeyValueItem> GenerateFontAwesome7Regular(
        DirectoryInfo resourcesFolder,
        DirectoryInfo outputEnumFolder)
    {
        ArgumentNullException.ThrowIfNull(resourcesFolder);
        ...
        return GoV7CssParseAndGenerate(
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"css\fontawesome7.css")),
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"fonts\fa7-regular-400.otf")),
            new FileInfo(Path.Combine(outputEnumFolder.FullName, @"FontAwesomeRegular7Type.cs")),
            "fa");
    }

    public static IList<LogKeyValueItem> GenerateFontAwesome7Solid(
        DirectoryInfo resourcesFolder,
        DirectoryInfo outputEnumFolder)
    {
        ArgumentNullException.ThrowIfNull(resourcesFolder);
        ...
        return GoV7CssParseAndGenerate(
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"css\fontawesome7.css")),
            new FileInfo(Path.Combine(resourcesFolder.FullName, @"fonts\fa7-solid-900.otf")),
            new FileInfo(Path.Combine(outputEnumFolder.FullName, @"FontAwesomeSolid7Type.cs")),
            "fa");
    }
    ...
    public static IList<LogKeyValueItem> GoV7CssParseAndGenerate(
        ...
        string prefix)
    {
        ...
        var collectedKeyValues = GetAllV7FontClassesWithContentValue(
            text,
            typeface,
            prefix);
    ...
    private static Dictionary<string, string> GetAllV7FontClassesWithContentValue(
        string cssText,
        Typeface typeface,
        string prefix)
    {
        ...
        // v7 CSS format: .fa-icon-name {\n  --fa: "\xxxx";\n}
        var regex = new Regex(
            @"(" + Regex.Escape(prefix) + @"-[a-z0-9][a-z0-9-]*)\s*\{\s*--fa:\s*""(\\[0-9a-fA-F]+)\s*""\s*;\s*\}",
            RegexOptions.None,
            TimeSpan.FromMinutes(1));

        foreach (Match match in regex.Matches(cssText))
        {
            var className = match.Groups[1].Value;
            var unicodeValue = match.Groups[2].Value;
            if (!tmpKeyValueList.ContainsKey(className))
            {
                tmpKeyValueList.Add(className, $"\"{unicodeValue}\"");
            }
        }
