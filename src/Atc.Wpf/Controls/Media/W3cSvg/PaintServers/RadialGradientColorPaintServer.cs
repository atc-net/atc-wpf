...
{
    public RadialGradientColorPaintServer(
        ...
        : base(
            ...
            node)
    {
        Cx = SvgXmlUtil.AttrValue(
            node,
            "cx",
            0.5);
        Cy = SvgXmlUtil.AttrValue(
            node,
            "cy",
            0.5);
        Fx = SvgXmlUtil.AttrValue(
            node,
            "fx",
            Cx);
        Fy = SvgXmlUtil.AttrValue(
            node,
            "fy",
            Cy);
        R = SvgXmlUtil.AttrValue(
            node,
            "r",
            0.5);
    }
    ...
    public override string ToString()
        => $"{nameof(Cx)}: {Cx}, {nameof(Cy)}: {Cy}, {nameof(Fx)}: {Fx}, {nameof(Fy)}: {Fy}, {nameof(R)}: {R}";
}