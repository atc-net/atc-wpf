...
/// <summary>
/// Interaction logic for ColorsView.
/// </summary>
...
public partial class ColorsView : INotifyPropertyChanged
{
    ...
    {
        filterText = e.NewValue.Replace("AtcApps.Colors.", string.Empty, StringComparison.Ordinal);
        PopulateBrushes();
    }
        ...
        ICollection<BrushInfo> filteredBrushes)
    {
        foreach (var resourceDict in resourceDictionary.MergedDictionaries)
        {
            foreach (var dictKey in resourceDict.Keys)
            {
                ...
                if (resourceDict[dictKey] is not Color color)
                {
                    continue;
                }

                var displayName = key.Replace("AtcApps.Colors.", string.Empty, StringComparison.Ordinal);
